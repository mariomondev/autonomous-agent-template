...
- `sqlite3` CLI (pre-installed on macOS/Linux; the agent uses it to update feature status)
...
5. Updates database via SQL: `UPDATE features SET passes = 1 WHERE id = X` when verification succeeds
...
│   ├── db.sqlite          # Feature tracking database
│   ├── current_batch.json # Current session's features (generated by agent)
...
- Query database: `sqlite3 .autonomous/db.sqlite "SELECT * FROM features WHERE passes = 0 LIMIT 10;"`
